...
merged = merge(shape, s, all.x=T, by.x="msoa11cd", by.y="geozone")
head(merged)
```
...
library(viridis)
library(scales)
ggplot(data=merged, aes(fill=NegativePotential)) + geom_sf(color="black", lwd=0.2)  + scale_fill_viridis_c(option="viridis", trans=modulus_trans(0)) + theme_bw() + theme(axis.title = element_blank(), axis.ticks = element_blank(), axis.text = element_blank(),panel.grid.major = element_line(colour = "transparent"))
```
